...
Вещи в магазине одежды продаются разных цветов и размеров. Ноутбуки продаются в разных модификациях, которые отличаются размером оперативной памяти, размером SSD, моделью видеокарты и т.д. Назовем эти особенности **опциями**, а набор конкретных значений - **вариантом**. Например, у товара "Футболка" могут быть опции "Размер" и "Цвет", а варианты - "Футболка S Красная", "Футболка M Зеленая" и т.д.
...
Заводить несколько одинаковых значений в рамках одной опции так же не имеет смысла, поэтому нужно создать уникальный индекс по двум полям - `option_id`, `name`. Если поставить поле `option_id` на первое место, то индекс будет выполнять сразу две функции:
...
Перечисленного набора полей может не хватить из-за специфичной безнес-логики или пользовательского интерфейса. Например, для цветов понадобится хранить шестнадцатеричный код.
...
CREATE TABLE products (
  ...
  name character varying -- Назвние товара
);
```
...
(
  ...
  variant_id bigint -- Идентификатор варианта, которому принадлежи цена
);
```
...
Клиент покупает конкретный вариант товара с указанием количества, что определяет минимальную структуру для хранения строки в корзине.

Корзину можно рассматривать как частный случай заказа, который находится в определенном статусе. В таком случае, дополнительную таблицу для корзины создавать не нужно.
...
CREATE TABLE order_items (
  id bigserial PRIMARY KEY, -- Идентификатор строки заказа
  order_id bigint, -- Идентификатор заказа, которому принадлежит строка
  variant_id bigint, -- Идентификатор варианта в строке заказа
